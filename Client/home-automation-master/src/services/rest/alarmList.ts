...
export const fetchAlarmListEntries = (token: string | null, filter: string) =>
  endpoint
    .get<AlarmListEntries | ErrorMessage>(
      `${config.getAlarmListControllerURI}` + GetUrl(filter),
...
export const fetchAlarmListEntry = (token: string | null, id: string) =>
  endpoint
    .get<AlarmListEntry | ErrorMessage>(
      `${config.getAlarmListControllerURI}` + "GetAlarm?id=" + id,
      { headers: createAuthenticationHeader(token) }
    )
    .then((r) => {
      if (r.status >= 300) {
...
export const AcknowledgeEntry = (
  token: string | null,
  id: string,
  data: string | ""
) =>
  endpoint
    .patch<AlarmListEntry | ErrorMessage>(
      `${config.getAlarmListControllerURI}AcknowledgeAlarm?AlarmID=` + id,
      "",
      { headers: createAuthenticationHeader(token) }
    )
    .then((r) => {
