...
export const login = (loginData: LoginData) =>
  endpoint
    .post<AuthenticationResponse>("User/Login", loginData, { timeout })
    .then(({ data: { authentication, user } }) => {
      return Promise.all([
        user,
        authentication,
        Storage.set({
          key: "user",
          value: JSON.stringify(user && typeof user === "object" ? user : {}),
        }),
        Storage.set({
          key: "authentication",
...
export const isNotExpired = (token: authentication | null | undefined) => {
...
export const loadUserData = () =>
  Promise.all([
    ...
    authentication: authentication.value
      ? JSON.parse(authentication.value)
      : null,
  }));
